...
- 开发语言：C++11，使用现代 C++ 特性（类、命名空间、enum class 等），保持轻量级设计；
...
|src/driver/barometer/|气压计|Barometer 类：init()、read()|
|src/driver/temp_hum/|温湿度传感器|TempHum 类：init()、read()|
...
# 1. LED 测试
bsp_tool led set led0 on          # 打开 LED0（设备名 led0，对应 /dev/led0）
...
# 3. 按键测试
bsp_tool key read key0            # 读取按键 key0 状态（0-未按下，1-按下）
bsp_tool key register key0 100    # 注册按键 key0 回调，检测周期 100ms
...
### 3.5.1 LED 头文件（src/driver/led/led.h）
...
int main() {
    ...
    if (ret != ErrorCode::Ok) {
        std::cerr << "LED init failed: " << errorToString(ret) << std::endl;
        return 1;
    }
